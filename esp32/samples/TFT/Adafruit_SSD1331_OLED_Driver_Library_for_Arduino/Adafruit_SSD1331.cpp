...
void Adafruit_SSD1331::begin(void) {
    ...
    if (_sclk) {
        ...
        sclkportreg =(unsigned char*) portOutputRegister(digitalPinToPort(_sclk));
        sclkpin = digitalPinToBitMask(_sclk);
        
        pinMode(_sid, OUTPUT);
        sidportreg =(unsigned char*) portOutputRegister(digitalPinToPort(_sid));
    ...
    csportreg =(unsigned char*) portOutputRegister(digitalPinToPort(_cs));
    
    rspin = digitalPinToBitMask(_rs);
    rsportreg =(unsigned char*) portOutputRegister(digitalPinToPort(_rs));
